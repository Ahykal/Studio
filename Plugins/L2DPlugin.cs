...
namespace Plugins.Ahykal
{
    public class L2DPlugin : IAssetPlugin
    {
        ...
        public void Run(PluginConnection connection)
        {
            ...
            this.SetCommand(new ToolStripMenuItem("SaveAllByAnimators")).Click += delegate
            {
                connection.Mainform.OpenFolderDelegate(o =>
                {
                    Live2DExtractor.SaveAllByAnimator(AssetList.ExportableAssets.Where(x => x.Type == ClassIDType.Animator), o.Folder, withPathIDCheck.Checked);
                });
            };
